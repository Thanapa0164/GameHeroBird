...
public class play1 extends JPanel implements ActionListener {
    ...
    private final ImageIcon imgmap1 = new ImageIcon(this.getClass().getResource("m1.jpg"));
...
//	JButton BStartV = new JButton(restartV);
	...
	public ArrayList<EBird4> Eb4 = new ArrayList<EBird4>();
	public ArrayList<EBird5> Eb5 = new ArrayList<EBird5>();
        public ArrayList<EBird6> Eb6 = new ArrayList<EBird6>();
	public ArrayList<coin2> coin2 = new ArrayList<coin2>();
        
        public int times ;
	public int HP = 10;
        boolean timestart = true;
	boolean startEBird4 = false;
	...
	int time_paralyze=5;
        ...
        Thread tEBird4 = new Thread(new Runnable(){
            public void run() {
                while(true){
                	try{
                            if(startEBird4 == false){
				Thread.sleep(2000);
                            }
        ...
        Thread tEBird5 = new Thread(new Runnable(){
            public void run() {
		while(true){
			try{
                            if(startEBird4==false){
        ...
        Thread tEBird6 = new Thread(new Runnable(){
            public void run() {
		while(true){
			try{
                            if(startEBird4==false){
	...
	Thread tcoin2 = new Thread(new Runnable(){
            public void run() {
            	while(true){
			try{
                            if(startEBird4==false){
...
//                this.add(BStartD);
		g.setColor(Color.BLACK);
                ...
                g.drawString("SCORE   "+scor,310,550);	
            }else if(times <= 0){
                this.setLayout(null);
...
//                this.add(BStartD);
		g.setColor(Color.BLACK);
                ...
                g.drawImage(imgmap1.getImage(),0,0,900,700,this);
		...
		if(h.y>550){
		    ...
		    if(ba.x>850){
                ...
                //========================================EBird4================= 
                for(int i=0 ; i<Eb4.size();i++){
                    g.drawImage(Eb4.get(i).getImage(),Eb4.get(i).getX(),Eb4.get(i).getY(),100,100,this);
 		}
		for(int i=0 ; i<fire.size();i++){
		    for(int j=0 ; j<Eb4.size();j++){
		    	if(Intersect(fire.get(i).getbound(),Eb4.get(j).getbound())){
			    Eb4.remove(j);
			    fire.remove(i);
			    scor += 5;
			}
                ...
                for(int i=0;i<Eb4.size();i++){
                    if(Intersect(h.getbound(),Eb4.get(i).getbound())){
                     Eb4.remove(i);
                     HP -= 1;
                    }
                }
                //========================EBird5=========================
		for(int i=0 ; i<Eb5.size();i++){
		    g.drawImage(Eb5.get(i).getImage(),Eb5.get(i).getX(),Eb5.get(i).getY(),100,100,this);
		 }
		for(int i=0 ; i<fire.size();i++){
		    for(int j=0 ; j<Eb5.size();j++){
		    	if(Intersect(fire.get(i).getbound(),Eb5.get(j).getbound())){
			    Eb5.remove(j);
			    fire.remove(i);
			    scor += 10;
			 } 
                ...
                for(int i=0;i<Eb5.size();i++){
                    if(Intersect(h.getbound(),Eb5.get(i).getbound())){
                     Eb5.remove(i);
                     HP -= 1;
                    }
                }
                //========================EBird6=========================
		for(int i=0 ; i<Eb6.size();i++){
		    g.drawImage(Eb6.get(i).getImage(),Eb6.get(i).getX(),Eb6.get(i).getY(),100,100,this);
		 }
		for(int i=0 ; i<fire.size();i++){
		    for(int j=0 ; j<Eb6.size();j++){
		    	if(Intersect(fire.get(i).getbound(),Eb6.get(j).getbound())){
			    Eb6.remove(j);
			    fire.remove(i);
			    scor += 10;
			 } 
                ...
                for(int i=0;i<Eb6.size();i++){
                    if(Intersect(h.getbound(),Eb6.get(i).getbound())){
                     Eb6.remove(i);
                     HP -= 1;
                    }
                }
		...
		for(int i=0 ; i<coin2.size();i++){
		    g.drawImage(coin2.get(i).getImage(),coin2.get(i).getX(),coin2.get(i).getY(),100,100,this);
		}
		for(int i=0 ; i<fire.size();i++){
		    for(int j=0 ; j<coin2.size();j++){
		    	if(Intersect(fire.get(i).getbound(),coin2.get(j).getbound())){
			    coin2.remove(j);
			    fire.remove(i);
			    HP-=1;
			}
                        if(Intersect(h.getbound(),coin2.get(j).getbound())){
                         coin2.remove(j);
                         scor += 20;
                        }
		    }
                ...
                for(int i=0;i<coin2.size();i++){
                    if(Intersect(h.getbound(),coin2.get(i).getbound())){
                     coin2.remove(i);
                     scor += 15;
                    }
                }
